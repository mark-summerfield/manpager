...
oo::define ConfigForm method make_widgets {} {
    ...
    # TODO manpages path
    # TODO Start at (*) Random Page ( ) Last Viewed Page
    ttk::label .config.configFileLabel -text "Config file"
    ttk::label .config.configFilenameLabel -relief sunken \
        -text [$Cfg filename]
...
oo::define ConfigForm method make_layout {} {
    ...
    grid .config.scaleSpinbox -row 0 -column 1 -sticky we {*}$opts
    grid .config.fontButton -row 1 -column 0 -sticky w {*}$opts
    grid .config.fontLabel -row 1 -column 1 -sticky we {*}$opts
    grid .config.blinkCheckbutton -row 2 -column 1 -sticky we
    grid .config.configFileLabel -row 3 -column 0 -sticky we {*}$opts
    grid .config.configFilenameLabel -row 3 -column 1 -sticky we {*}$opts
    grid .config.buttons -row 9 -column 0 -columnspan 2 -sticky we
...
oo::define ConfigForm method on_font_chosen args {
    if {[llength $args] > 0} {
        ...
        if {[llength $args] > 1} {
            ...
            puts "############# chose font family=$family size=$size" ;#TODO
        }
    }
}
